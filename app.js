...
const handleMessage = async (msg) => {
    ...
    const user = {
        id: msg.from,                 // unique ID (always works)
        number: contact.number || "", // may be undefined
    ...
    const ADMIN = '919933522250';
    console.log("trying to enter admin block:", user);
    if (user.number === ADMIN) {
        console.log("in admin block:", user)
        // Call next token
        if (text === 'next') {
